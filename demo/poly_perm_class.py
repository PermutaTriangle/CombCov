    ...
    def get_subrules(self):
        ...
        if self.tiling.dimensions != (1, 1) or self.tiling.requirements:
            raise NotImplementedError(
                            "Only handle 1x1 tilings without requirements :(")
        ...
        for i in range(1, maxlen + 1):
            for perm in Av(basis).of_length(i):
                ...
                for assignment in itertools.product(*[blocks(cell)
                                                    for cell in cells]):
                    obstructions = []
                    requirements = []
                    ...
                    res.append(CombCovTiling(
                                        Tiling(obstructions, requirements)))
